...
from utils.func import  make_sparse_camera,  make_round_views, make_sphere
from utils.render import NormalsRenderer,LocalNormaRender
...
class ReMesh:
    ...
    def optimize_case(self, case, pose, clr_img, nrm_img, opti_texture=True):
        ...
        for i in tqdm(range(self.opt.iters)):
            ...
            normals = nrm[..., :3]   
            if i%50 == 0:
                Image.fromarray((normals[0].detach().cpu().numpy()*255).astype(np.uint8)).save(f'{case_path}/normal_rendered.png')
            # if i < 800:
